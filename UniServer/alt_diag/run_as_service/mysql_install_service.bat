...
TITLE UNIFORM SERVER - Install MySQL Service

rem ###################################################
rem # Name: mysql_install_service.bat
...
:strCmd = us_mysql_exe & " --install " & Chr(34) & service_name & Chr(34) & " --defaults-file=" & USF_MYSQL_INI
:wscript.echo strCmd
rem ### END VBScript ###

rem -- run above script and get string returned
findstr "^:" "%~sf0">temp.vbs
for /f "delims=" %%N in ('cscript //nologo temp.vbs') do set install_service=%%N
del temp.vbs
